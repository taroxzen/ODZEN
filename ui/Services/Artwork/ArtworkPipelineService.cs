...
namespace Odzen.Avalonia.Services
{
    public static class ArtworkPipelineService
    {
        ...
        private static readonly string CustomStorageDir = Path.Combine(
            Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "ODZEN", "artwork", "custom");
        ...
        public static Bitmap? ResolveArtwork(string? exePath, string? installPath, string? storeId, string platform, string gameId, string gameName)
        {
            ...
            // 0. CHECK USER CUSTOM ARTWORK
            string customPng = Path.Combine(CustomStorageDir, $"{Sanitize(gameId)}.png");
            if (File.Exists(customPng))
            {
                try
                {
                    using var stream = File.OpenRead(customPng);
        ...
        private static string? MatchBuiltInArtworkBank(string platform, string gameId, string gameName)
        {
            ...
            return Path.Combine(assetsDir, "odzen_logo.ico");
        }
