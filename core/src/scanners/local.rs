...
fn try_add_game(
    ...
    let name = clean_local_game_title(&raw_name);
...
fn clean_local_game_title(raw: &str) -> String {
    let mut s = raw.to_string();
    let noise_tags = [
        "- 500MB", "- 500mb", "[FitGirl Repack]", "[DODI Repack]", "[DODI]", "[FitGirl]",
        "[EMPRESS]", "[CODEX]", "[RUNE]", "[CPY]", "[SKIDROW]", "[PLAZA]", "[FLT]",
        "v1.0", "v1.0.0", "v1.1", "v1.2", "v1.3", "v1.4", "v1.5", "v2.0",
        "Build.", "Repack", "Deluxe Edition", "Gold Edition", "Portable"
    ];
    for tag in noise_tags {
        if let Some(pos) = s.to_lowercase().find(&tag.to_lowercase()) {
            s.replace_range(pos..pos + tag.len(), "");
        }
    }
    let lower = s.trim().to_ascii_lowercase();
    if lower == "gta san andreas" || lower.starts_with("gta san andreas") || lower == "gtasa" {
        return "Grand Theft Auto: San Andreas".to_string();
    }
    if lower == "gta v" || lower.starts_with("gta 5") {
        return "Grand Theft Auto V".to_string();
    }
    if lower == "gta iv" || lower.starts_with("gta 4") {
        return "Grand Theft Auto IV".to_string();
    }
    if lower == "gta vice city" || lower.starts_with("gta vc") {
        return "Grand Theft Auto: Vice City".to_string();
    }
    s.trim().trim_matches(['-', '_', ' ']).to_string()
}
...
fn looks_like_game_dir(dir: &Path) -> bool {
    ...
    // Nested Unreal/Modern game folder (e.g. dir/Title/Binaries/Win64 or Content/Paks)
    if let Ok(entries) = fs::read_dir(dir) {
        for e in entries.flatten() {
            let sub = e.path();
            if sub.is_dir() {
                if sub.join("Binaries").join("Win64").is_dir() || sub.join("Content").is_dir() {
                    if util::find_main_exe(dir).is_some() {
                        return true;
                    }
                }
            }
...
fn scan_uninstall_registry(
    ...
    for (hive, reg_path) in paths {
        for e in enum_uninstall_entries(hive, reg_path) {
            ...
            seen.insert(key);
            let clean_display = clean_local_game_title(&display);
            let store_id = format!("reg-{}", slug_id(&install));
            let mut game = Game::new(Platform::Local, &store_id, clean_display);
