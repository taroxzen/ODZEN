...
using Onyx.Avalonia.Services;

namespace Onyx.Avalonia.ViewModels
{
    public partial class MainViewModel : ObservableObject
    {
        ...
        [ObservableProperty] private string _txtDirectXHud = "DIRECTX 12 HUD";
        ...
        [ObservableProperty] private string _txtAppSettingsSubtitle = "ONYX Launcher sistem tercihlerini ve yapay zeka oyun bulucu parametrelerini yapılandırın.";
        ...
        [ObservableProperty] private string _txtAiEngine = "Yapay Zeka Oyun Bulucu Motoru";
        [ObservableProperty] private string _txtAiDetection = "Yapay Zeka Oyun Tespiti";
        [ObservableProperty] private string _txtAiDetectionDesc = "Sistem kaynak kullanımına göre çalışan oyunu otomatik tanır ve bildirim verir";
        ...
        [ObservableProperty] private string _txtAiPromptTitle = "Yapay Zeka Yeni Oyun Algıladı!";
        [ObservableProperty] private string _txtYesAdd = "Evet, Ekle";
        [ObservableProperty] private string _txtNoDismiss = "Hayır, Geç";
        ...
        // AI Game Detection Prompt (Evet / Hayır Bildirimi)
        ...
        [ObservableProperty]
        private HardwareStats _hwStats = new();
        ...
        public MainViewModel()
        {
            ...
            _hudTimer = new DispatcherTimer { Interval = TimeSpan.FromMilliseconds(1500) };
            _hudTimer.Tick += (s, e) => UpdateHardwareHud();
            _hudTimer.Start();
            UpdateHardwareHud();
        ...
        private void UpdateHardwareHud()
        {
            HwStats = _hardwareMonitor.GetCurrentStats();
        }
                    ...
                    try
                    {
                        ...
                        if (IsGameProcess(pName))
                        {
                            ...
                            var existing = AllGames.FirstOrDefault(g => 
                                g.Name.Equals(title, StringComparison.OrdinalIgnoreCase) || 
                            ...
                            if (existing != null)
                            {
                                ...
                                ShowToastNotification($"🎮 {existing.Name} çalıştırıldı ve kütüphane güncellendi.");
                            }
                            else
                            {
                                // Yeni oyun: Kullanıcıya Evet / Hayır onay sekmesi aç!
                                DetectedGameName = title;
                                DetectedGameExe = exe;
                                DetectedGameProcessName = proc.ProcessName;
                                IsAiDetectionPromptOpen = true;

                                // Windows Sağ Alt Sistem Bildirimi (Windows Tray / Toast - 8 Dil Destekli)
                                WindowsNotificationService.ShowGameDetectedNotification(LocalizationService.CurrentLanguage, title);
                            }
                            break;
                        }
                    }
        ...
        public void ConfirmAddDetectedGame()
        {
            ...
            ShowToastNotification($"✅ {DetectedGameName} kütüphanenize başarıyla eklendi!");
        }
        ...
        {
            IsAiDetectionPromptOpen = false;
            ShowToastNotification($"🚫 {DetectedGameName} kütüphaneye eklenmedi.");
        }

        private static bool IsGameProcess(string processName)
        {
            string[] gameSignatures = { "valorant", "fc26", "fifa", "fortnite", "marvel", "r6siege", "rainbowsix", "projectzomboid", "rinamt2", "metin2", "discovery", "gta_sa", "gta5", "donutcounty", "minecraft", "csgo", "cs2", "pubg" };
        ...
        private void CheckAutostartStatus()
        {
            try
            {
                ...
                _autostart = key?.GetValue("ONYXLauncher") != null;
        ...
        private static void ApplyAutostartRegistry(bool enable)
        {
            try
            {
                ...
                {
                    key.SetValue("ONYXLauncher", $"\"{exePath}\"");
                }
                else
                {
                    key.DeleteValue("ONYXLauncher", false);
                }
            }
            catch { }
        }
        ...
        public void SaveGameConfig()
        {
            ...
            ApplyFilter();
            IsGameDetailOpen = false;
            ShowToastNotification($"💾 '{SelectedGame.Name}' yapılandırması başarıyla kaydedildi!");
        }
        ...
        partial void OnSelectedLanguageIndexChanged(int value)
        {
            ...
            switch (lang)
            {
                case AppLanguage.German: // Almanca
                    ...
                    TxtAppSettingsSubtitle = "Konfiguriere ONYX Launcher Systemeinstellungen und KI-Spielerkennung.";
                    ...
                    TxtAiEngine = "KI-Spielerkennungs-Engine";
                    TxtAiDetection = "KI-Spielerkennung";
                    TxtAiDetectionDesc = "Fragt per Benachrichtigung (Ja / Nein), wenn ein neues Spiel startet";
                    ...
                    TxtAiPromptTitle = "KI hat ein neues Spiel erkannt!";
                ...
                case AppLanguage.Bulgarian: // Bulgarca
                    ...
                    TxtAppSettingsSubtitle = "Конфигурирайте системните предпочитания на ONYX Launcher.";
                    ...
                    TxtAiEngine = "AI двигател за намиране на игри";
                    TxtAiDetection = "AI откриване на игри";
                    TxtAiDetectionDesc = "Пита с известие (Да / Не), когато стартира нова игра";
                    ...
                    TxtAiPromptTitle = "AI откри нова игра!";
                    TxtYesAdd = "Да, добави";
                ...
                case AppLanguage.Spanish: // İspanyolca
                    ...
                    TxtAppSettingsSubtitle = "Configura las preferencias del sistema y detección por IA de ONYX Launcher.";
                    ...
                    TxtAiEngine = "Motor de detección de juegos por IA";
                    TxtAiDetection = "Detección de juegos por IA";
                    TxtAiDetectionDesc = "Pregunta con notificación (Sí / No) al iniciar un juego nuevo";
                    ...
                    TxtAiPromptTitle = "¡La IA detectó un juego nuevo!";
                    TxtYesAdd = "Sí, añadir";
                ...
                case AppLanguage.Dutch: // Hollandaca
                    ...
                    TxtAppSettingsSubtitle = "Configureer ONYX Launcher systeemvoorkeuren en AI-gamedetectie.";
                    ...
                    TxtAiEngine = "AI-gamefinder-engine";
                    TxtAiDetection = "AI-gamedetectie";
                    TxtAiDetectionDesc = "Vraagt via melding (Ja / Nee) wanneer een nieuwe game start";
                    ...
                    TxtAiPromptTitle = "AI heeft een nieuwe game gedetecteerd!";
                ...
                case AppLanguage.French: // Fransızca
                    ...
                    TxtAppSettingsSubtitle = "Configurez les préférences système et la détection IA d'ONYX Launcher.";
                    ...
                    TxtAiEngine = "Moteur de détection de jeux par IA";
                    TxtAiDetection = "Détection de jeux par IA";
                    TxtAiDetectionDesc = "Demande par notification (Oui / Non) au lancement d'un nouveau jeu";
                    ...
                    TxtAiPromptTitle = "L'IA a détecté un nouveau jeu !";
                ...
                case AppLanguage.Russian: // Rusça
                    ...
                    TxtAppSettingsSubtitle = "Настройте параметры системы ONYX Launcher и обнаружение игр ИИ.";
                    ...
                    TxtAiEngine = "ИИ-движок поиска игр";
                    TxtAiDetection = "ИИ-обнаружение игр";
                    TxtAiDetectionDesc = "Спрашивает через уведомление (Да / Нет) при запуске новой игры";
                    ...
                    TxtAiPromptTitle = "ИИ обнаружил новую игру!";
                ...
                case AppLanguage.English: // İngilizce
                    ...
                    TxtAppSettingsSubtitle = "Configure ONYX Launcher system preferences and AI detection parameters.";
                    ...
                    TxtAiEngine = "AI Game Finder Engine";
                    TxtAiDetection = "AI Game Detection";
                    TxtAiDetectionDesc = "Prompts with notification to add to library when a new game starts (Yes / No)";
                    ...
                    TxtAiPromptTitle = "AI Detected a New Game!";
                    TxtYesAdd = "Yes, Add";
                    TxtNoDismiss = "No, Skip";
                    TxtLaunch = "Launch";
                ...
                default: // Türkçe
                    ...
                    TxtAppSettingsSubtitle = "ONYX Launcher sistem tercihlerini ve yapay zeka oyun bulucu parametrelerini yapılandırın.";
                    ...
                    TxtAiEngine = "Yapay Zeka Oyun Bulucu Motoru";
                    TxtAiDetection = "Yapay Zeka Oyun Tespiti";
                    TxtAiDetectionDesc = "Arka planda yeni bir oyun açıldığında bildirimle sorar (Evet / Hayır) ve kütüphaneye ekler";
                    ...
                    TxtAiPromptTitle = "Yapay Zeka Yeni Oyun Algıladı!";
                    TxtYesAdd = "Evet, Ekle";
                    TxtNoDismiss = "Hayır, Geç";
                    TxtLaunch = "Başlat";
